class SvgIcon
  attr_reader :name, :variant, :class_names, :path, :data_attributes, :html_attributes
  ...
  def render
    ...
    if html_attributes.present?
      html_attributes.each do |key, attr_value|
        svg.set_attribute(key.to_s.dasherize, attr_value)
      end
    end
